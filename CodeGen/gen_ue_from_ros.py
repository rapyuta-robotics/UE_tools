...
def get_types_cpp(target_paths):
    ...
    for key, value in types_dict.items():
        ...
        for v in value:
            ...
            for t_ue, v_ue in res_ue.items():
                type_ue = t_ue[0]
                var_ue = f'{t_ue[1]}s' if ('TArray' in type_ue) else t_ue[1]
                size = 0
                ...
                var_initialization = f' = {dft_val}' if (
                    dft_val != None) else ''
                var_declaration = f'{type_ue} {v_ue}{var_initialization};\n\n\t'
                if (('/') in type_ue):
                    cpp_type += var_declaration
                elif (('unsigned int' in type_ue) or
                     ('double' in type_ue) or
                     ('int8' in type_ue) or
                     ('int16' in type_ue) or
                     ('uint16' in type_ue) or
                     ('uint64' in type_ue)):
...
if __name__ == "__main__":
    ...
    def print_group_name_info(in_group_name):
        print(
            f"types_cpp[{in_group_name}] size:{len(types_cpp[in_group_name]) if (in_group_name in types_cpp) else 0}")

    # generate code
    # Refer to ros2 msgs pkg
    ...
    for p in range(len(ue_paths)):
        ...
        if not os.path.exists(subdir_path):
            continue
        print('[ROS2 PKG SHARE PATH]', subdir_path)
